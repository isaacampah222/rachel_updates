    ...
    while ($moddir = readdir($handle)) {
        ...
        if (is_dir("$absModPath/$moddir")) { // look in dirs only
            ...
            if ($fragment) { // check for index fragment
                ...
                preg_match("/<!--\s*category\s*=\s*(?:\"|')?([^\"'\s]+?)(?:\"|')?\s*-->/", $content, $match);
                if (isset($match[1])) { $category = $match[1]; }
...
function showip () {
    ...
    if(is_rachelpi()){
        ...
        if (isset($match[1])) { echo "<b>WIFI</b>: $match[1] "; }
        return;
    }
...
function updateRequest($content) {
    return file_put_contents('/var/www/request.txt', $content, FILE_APPEND);
//    if (file_put_contents('/var/www/request.txt', $content, FILE_APPEND)){
//     echo "console.log('" . "successfully created" . "');";
//    } else {
